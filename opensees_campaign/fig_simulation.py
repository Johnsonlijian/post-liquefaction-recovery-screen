...
fig, (axA, axB, axC) = plt.subplots(1, 3, figsize=(13.2, 4.25))
fig.subplots_adjust(left=0.055, right=0.985, bottom=0.135, top=0.86, wspace=0.34)
...
axA.set_title("(A)  Calibrated effective-stress engine", fontsize=9.5, loc="left", fontweight="bold")
axA.legend(fontsize=7, frameon=False, ncol=2, loc="upper right", handlelength=1.5, columnspacing=1.0)
axA.text(
    0.03,
    0.04,
...
axB2.set_ylabel(r"Vertical effective stress  $\sigma'_v$ (kPa)", color=C_BASE)
axB2.tick_params(axis="y", colors=C_BASE)
axB2.set_ylim(-3, 70)
axB.set_title("(B)  Effective-stress collapse under one event", fontsize=9.5, loc="left", fontweight="bold")
axB.annotate(
...
axC.set_title("(C)  The demand-recovery race", fontsize=9.5, loc="left", fontweight="bold")
...
fig.suptitle(
    ...
    "while recovery (densification) is far too slow to intervene",
    fontsize=10.3,
    y=0.975,
    fontweight="bold",
)
