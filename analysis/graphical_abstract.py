"""Graphical abstract for the Engineering Geology submission.

Elsevier graphical abstracts should be at least 1328 x 531 px and remain
legible at reduced size. This script exports a 2250 x 900 px, 2.5:1 image.
"""
...
evx = [0, 5.6, 9.3, 15.6]
evnm = ["Sep'10", "Feb'11", "Jun'11", "Dec'11"]
for x in evx:
...
ax.text(16.5, demand + 0.03, "seismic demand", color=C["dem"], fontsize=7.6, ha="right", va="bottom", weight="bold")
t = np.linspace(0, 16.3, 500)
floor, rate = 0.40, 0.030
...
ax.set_title("Reserve rebuilds far too slowly", fontsize=8.8, weight="bold", color=C["ink"], loc="left", pad=12)

# Divider.
fig.add_artist(plt.Line2D([0.505, 0.505], [0.13, 0.87], color="#dcdcdc", lw=1.1, transform=fig.transFigure))

# Right: takeaways.
ax2 = fig.add_axes([0.53, 0.0, 0.45, 1.0])
ax2.axis("off")
ax2.set_xlim(0, 1)
ax2.set_ylim(0, 1)
ax2.text(
...
for txt, col in bullets:
    ...
    y -= 0.125
ax2.text(0.0, 0.05, "Open CPT pipeline - 845 sites - 2010-2011 Canterbury, NZ", fontsize=7.2, color=C["gray"], va="bottom", style="italic")
